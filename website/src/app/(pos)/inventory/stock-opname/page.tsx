...
const mockStockOpname = [
  { id: 1, code: "SO240912001", date: "12/09/2024", warehouse: "Gudang Utama", itemCount: 45, systemStock: 500, actualStock: 495, difference: -5, status: "draft" },
  { id: 2, code: "SO240912002", date: "12/09/2024", warehouse: "Gudang Cab. Bandung", itemCount: 30, systemStock: 200, actualStock: 200, difference: 0, status: "completed" },
  { id: 3, code: "SO240911001", date: "11/09/2024", warehouse: "Gudang Utama", itemCount: 50, systemStock: 450, actualStock: 445, difference: -5, status: "draft" },
  { id: 4, code: "SO240910001", date: "10/09/2024", warehouse: "Gudang Utama", itemCount: 40, systemStock: 380, actualStock: 380, difference: 0, status: "completed" },
];
...
export default function StockOpnamePage() {
  ...
  const filteredData = mockStockOpname.filter((p) =>
  ...
  const totalItemCount = filteredData.reduce((acc, p) => acc + p.itemCount, 0);
  const totalDifference = filteredData.reduce((acc, p) => acc + p.difference, 0);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "draft":
        return <span className="rounded-full px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-700">Draft</span>;
      default:
  ...
  return (
    <PageWrapper className="bg-gray-100">
      <PageTitle
        ...
        subtitle="Kelola stock opname gudang"
        actions={
          <div className="flex gap-2">
            ...
            <Button size="sm" className="bg-[#9C27B0] hover:bg-[#7B1FA2]">
              <Plus className="size-4 mr-2" /> Opname Baru
            </Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        <div className="bg-white rounded-lg border border-gray-200 px-4 py-3">
          <div className="text-xs text-gray-500 mb-1">Total Opname</div>
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-gray-900">{totalOpname}</span>
            <span className="text-xs text-gray-500">Transaksi</span>
          </div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 px-4 py-3">
          <div className="text-xs text-gray-500 mb-1">Total Item Diopname</div>
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-gray-900">{totalItemCount}</span>
            <span className="text-xs text-gray-500">Items</span>
          </div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 px-4 py-3">
          <div className="text-xs text-gray-500 mb-1">Total Selisih</div>
          <div className="flex items-baseline gap-2">
            <span className={cn("text-xl font-bold", totalDifference < 0 ? "text-red-600" : "text-green-600")}>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr className="text-xs text-gray-500 text-left">
                ...
                <th className="px-4 py-3 font-medium text-right">Jumlah Item</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedData.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">{item.itemCount}</td>
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">{item.systemStock}</td>
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">{item.actualStock}</td>
                  <td className={cn("px-4 py-3 text-sm font-medium text-right", item.difference < 0 ? "text-red-600" : item.difference > 0 ? "text-green-600" : "text-gray-700")}>
