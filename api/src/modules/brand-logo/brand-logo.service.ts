...
@Injectable()
export class BrandLogoService extends BaseService<
  any,
  CreateBrandLogoDto,
  UpdateBrandLogoDto
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateBrandLogoDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.BrandLogo.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:brand-logo:*');
  //
  //     return result;
  //   });
  // }
