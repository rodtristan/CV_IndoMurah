import { Controller, Get, Put, Post, Delete, Param, Body, Query, UseGuards, ParseIntPipe, NotFoundException } from '@nestjs/common';
...
export class UserController {
  ...
  @ApiQuery({ name: '$select', required: false, description: 'Select fields: id,full_name,email' })
  @ApiQuery({ name: '$include', required: false, description: 'Include relations: role,userRoles' })
  @ApiQuery({ name: '$where[is_active]', required: false, description: 'Filter: true/false' })
  ...
  async assignRole(@Param('id', ParseIntPipe) id: number, @Body() dto: AssignRoleDto) {
    const data = await this.userService.assignRole(id, dto.role_id);
    return ApiResponse.ok(data, 'Role assigned to user');
  }

  @Delete(':id/roles/:roleId')
  @ApiOperation({ summary: 'Revoke an extra role from a user' })
  async revokeRole(@Param('id', ParseIntPipe) id: number, @Param('roleId', ParseIntPipe) roleId: number) {
