...
@Injectable()
export class DailySalesSummaryService extends BaseService<
  any,
...
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateDailySalesSummaryDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.DailySalesSummary.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:daily-sales-summary:*');
  //
  //     return result;
  //   });
  // }
