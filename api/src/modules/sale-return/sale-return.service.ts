...
export class SaleReturnService {
    ...
    return this.redis.getOrSet(
      ...
      async () => {
        const prismaQuery = this.queryService.buildPrismaQuery(query, {
          ...
          allowedIncludes: ['sale', 'customer', 'warehouse', 'creator', 'returnItems', 'returnItems.product'],
  ...
  async findOne(id: number, query: Record<string, any> = {}) {
    ...
    return this.redis.getOrSet(
      ...
      async () => {
        const prismaQuery = this.queryService.buildPrismaQuery(query, {
          allowedIncludes: ['sale', 'customer', 'warehouse', 'creator', 'returnItems', 'returnItems.product', 'returnItems.unit'],
        });
  ...
  async create(dto: CreateSaleReturnDto, userId: string) {
    ...
    const saleReturn = await this.prisma.saleReturn.create({
      data: {
        code,
        saleId: dto.sale_id,
        customerId: dto.customer_id || sale.customerId,
  ...
  async update(id: number, dto: UpdateSaleReturnDto) {
    ...
    if (saleReturn.status !== TransactionStatus.DRAFT) {
