...
@Injectable()
export class PurchaseReturnItemService extends ReadOnlyItemService<
  any,
...
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreatePurchaseReturnItemDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.PurchaseReturnItem.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:purchase-return-item:*');
  //
  //     return result;
  //   });
  // }
