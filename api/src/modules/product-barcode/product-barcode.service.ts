...
@Injectable()
export class ProductBarcodeService extends BaseService<
  any,
...
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateProductBarcodeDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.ProductBarcode.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:product-barcode:*');
  //
  //     return result;
  //   });
  // }
