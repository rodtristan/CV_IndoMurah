...
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateProductStockDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.ProductStock.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:product-stock:*');
  //
  //     return result;
  //   });
  // }
