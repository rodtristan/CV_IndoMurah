...
export class PurchasePaymentService {
  ...
  async setClearedBatch(items: { ID: number; IsCleared: boolean; ClearedAt?: string | null }[], userId?: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      for (const it of items ?? []) {
        const p = await tx.purchasePayment.findUnique({ where: { ID: Number(it.ID) } });
        ...
        await tx.purchasePayment.update({ where: { ID: p.ID }, data: { IsCleared: cleared, ClearedAt: at } });
        await this.afterPaymentWrite(tx, p.ID, userId ?? p.CreatedByID);
        parents.add(p.PurchaseID);
      }
    }, { timeout: 60000 });
  ...
  // ═══ Dokumen "Bayar Hutang" (Ketoko): satu nomor, banyak faktur ═══════════════

  private async nextBatchCode(tx: Tx, prefix: string) {
    const d = new Date();
    const head = `${prefix}-${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}-`;
    const last = await tx.purchasePayment.findFirst({ where: { BatchCode: { startsWith: head } }, orderBy: { BatchCode: 'desc' }, select: { BatchCode: true } });
    const seq = last?.BatchCode ? Number(last.BatchCode.slice(head.length)) + 1 : 1;
    return `${head}${String(seq).padStart(4, '0')}`;
  }

  /** Faktur pembelian supplier yang masih ada sisa hutang (untuk form Bayar Hutang). */
  async outstanding(supplierId: number) {
    const rows = await this.prisma.purchase.findMany({
      where: { SupplierID: supplierId, Remaining: { gt: 0 }, Status: { Code: { not: 'CANCELLED' } }, PaymentStatus: { Code: { not: 'CANCELLED' } } },
      orderBy: [{ DueDate: 'asc' }, { Date: 'asc' }],
      select: { ID: true, Code: true, Date: true, DueDate: true, Total: true, Paid: true, Remaining: true },
    });
    return rows.map((r) => this.serializeDeep(r));
  }
  ...
  async createBatch(dto: CreatePaymentBatchDto, userId: string) {
    ...
    const code = await this.prisma.$transaction(async (tx) => {
      const batchCode = await this.nextBatchCode(tx, 'BH');
      const date = dto.Date ? new Date(dto.Date) : new Date();
      for (const line of lines) {
        const pu = await tx.purchase.findUnique({
          where: { ID: line.InvoiceID },
          include: { PurchasePayments: true, PurchaseReturns: { select: { TotalReturn: true, Status: { select: { Code: true } } } } },
        });
        if (!pu || pu.SupplierID !== dto.PartnerID) throw new BadRequestException(`Faktur #${line.InvoiceID} bukan milik supplier ini`);
        const committed = pu.PurchasePayments.reduce((a, x) => a + Number(x.Amount), 0);
        const remaining = Number(pu.Total) - this.returnsOf(pu) - committed;
        const want = (line.Amount || 0) + (line.Discount || 0);
        if (want > remaining + 0.005) throw new BadRequestException(`Pembayaran ${pu.Code} (${want}) melebihi sisa hutang (${Math.max(remaining, 0)})`);
        ...
        for (const part of parts) {
          ...
          const created = await tx.purchasePayment.create({
            data: {
              PurchaseID: pu.ID, MethodID: methodId, Amount: new Prisma.Decimal(part.amount), ReferenceNumber: dto.Number || null,
  ...
  async getBatch(code: string) {
    ...
    const rows = await this.prisma.purchasePayment.findMany({
      ...
      include: { Purchase: { select: { ID: true, Code: true, Date: true, DueDate: true, Total: true, Remaining: true, Supplier: { select: { ID: true, Code: true, Name: true } } } }, Method: true, Creator: { select: { Username: true } } },
    });
    if (!rows.length) throw new NotFoundException('Dokumen pembayaran tidak ditemukan');
    const first = rows[0];
    const byInvoice = new Map<number, any>();
    for (const r of rows) {
      const cur = byInvoice.get(r.PurchaseID) ?? { InvoiceID: r.PurchaseID, Code: r.Purchase.Code, Date: r.Purchase.Date, DueDate: r.Purchase.DueDate, Total: Number(r.Purchase.Total), Remaining: Number(r.Purchase.Remaining), Amount: 0, Discount: 0 };
      if (r.InstrumentType === 'DISCOUNT') cur.Discount += Number(r.Amount);
      else cur.Amount += Number(r.Amount);
      byInvoice.set(r.PurchaseID, cur);
    }
    const lines = [...byInvoice.values()];
    return this.serializeDeep({
      BatchCode: first.BatchCode ?? `#${first.ID}`, Date: first.Date, Partner: first.Purchase.Supplier, Method: first.Method,
  ...
  async deleteBatch(code: string) {
    const where = code.startsWith('#') ? { ID: Number(code.slice(1)) } : { BatchCode: code };
    const rows = await this.prisma.purchasePayment.findMany({ where, select: { ID: true, PurchaseID: true } });
    if (!rows.length) throw new NotFoundException('Dokumen pembayaran tidak ditemukan');
    await this.prisma.$transaction(async (tx) => { for (const r of rows) await this.deleteTx(tx, r.ID); }, { timeout: 60000 });
    for (const r of rows) await this.invalidate(r.PurchaseID);
    return { code, deleted: rows.length };
  }

  /** Daftar Pembayaran Hutang: satu baris per dokumen (No Transaksi, Tanggal, Cara Bayar, Supplier, Total). */
  async listBatches(query: Record<string, any>) {
    ...
    if (query.partnerId) where.Purchase = { SupplierID: Number(query.partnerId) };
    if (query.search) {
    ...
    const rows = await this.prisma.purchasePayment.findMany({
      ...
      include: { Purchase: { select: { Code: true, Supplier: { select: { Code: true, Name: true } } } }, Method: { select: { Name: true } }, Creator: { select: { Username: true } } },
    });
    ...
    for (const r of rows) {
      ...
      const d = docs.get(key) ?? {
        ...
        PartnerCode: r.Purchase.Supplier?.Code, PartnerName: r.Purchase.Supplier?.Name, Notes: r.Notes, Number: r.ReferenceNumber,
      ...
      if (!d.Invoices.includes(r.Purchase.Code)) d.Invoices.push(r.Purchase.Code);
      docs.set(key, d);
    }
