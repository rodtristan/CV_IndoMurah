...
@Injectable()
export class ShelfProductService extends BaseService<
  any,
...
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateShelfProductDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.ShelfProduct.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:shelf-product:*');
  //
  //     return result;
  //   });
  // }
