...
@Injectable()
export class UserMenuService extends BaseService<
  any,
  CreateUserMenuDto,
  UpdateUserMenuDto
> {
  ...
  // Contoh:
  //
  // async processTransaction(data: CreateUserMenuDto, userId: string) {
  //   return this.prisma.$transaction(async (tx) => {
  //     // 1. Create record
  //     const result = await tx.UserMenu.create({ data });
  //
  //     // 2. Update related records
  //     // await tx.relatedModel.update(...);
  //
  //     // 3. Invalidate cache
  //     await this.redis.del('cache:user-menu:*');
  //
  //     return result;
  //   });
  // }
