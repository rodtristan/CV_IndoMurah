"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VoucherController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const voucher_service_1 = require("./voucher-service");
const voucher_dto_1 = require("./voucher.dto");
const jwt_auth_guard_1 = require("../../../common/guards/jwt-auth-guard");
const api_response_dto_1 = require("../../../common/dto/api-response-dto");
let VoucherController = class VoucherController {
    constructor(voucherService) {
        this.voucherService = voucherService;
    }
    async createVoucher(dto) {
        const userId = 'system';
        const data = await this.voucherService.createVoucher(dto, userId);
        return api_response_dto_1.ApiResponse.ok(data, 'Voucher created successfully');
    }
    async listVouchers(dto) {
        const data = await this.voucherService.listVouchers(dto);
        return api_response_dto_1.ApiResponse.ok(data);
    }
    async getVoucher(id) {
        const data = await this.voucherService.getVoucher(id);
        return api_response_dto_1.ApiResponse.ok(data);
    }
    async updateVoucher(id, dto) {
        const userId = 'system';
        const data = await this.voucherService.updateVoucher(id, dto, userId);
        return api_response_dto_1.ApiResponse.ok(data, 'Voucher updated');
    }
    async validateVoucher(dto) {
        const data = await this.voucherService.validateVoucher(dto);
        return api_response_dto_1.ApiResponse.ok(data);
    }
    async useVoucher(id) {
        const userId = 'system';
        const data = await this.voucherService.useVoucher(id, userId);
        return api_response_dto_1.ApiResponse.ok(data);
    }
    async deleteVoucher(id) {
        const userId = 'system';
        const data = await this.voucherService.deleteVoucher(id, userId);
        return api_response_dto_1.ApiResponse.ok(data, 'Voucher deactivated');
    }
};
exports.VoucherController = VoucherController;
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Create new voucher' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [voucher_dto_1.CreateVoucherDto]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "createVoucher", null);
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'List vouchers' }),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [voucher_dto_1.VoucherFilterDto]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "listVouchers", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Get voucher by ID' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "getVoucher", null);
__decorate([
    (0, common_1.Patch)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Update voucher' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, voucher_dto_1.UpDateVoucherDto]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "updateVoucher", null);
__decorate([
    (0, common_1.Post)('validate'),
    (0, swagger_1.ApiOperation)({ summary: 'Validate voucher code' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [voucher_dto_1.ValiDateVoucherDto]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "validateVoucher", null);
__decorate([
    (0, common_1.Post)(':id/use'),
    (0, swagger_1.ApiOperation)({ summary: 'Mark voucher as used' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "useVoucher", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Delete (deactivate) voucher' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], VoucherController.prototype, "deleteVoucher", null);
exports.VoucherController = VoucherController = __decorate([
    (0, swagger_1.ApiTags)('Voucher - Voucher/Promo'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Controller)('business-logic/voucher'),
    __metadata("design:paramtypes", [voucher_service_1.VoucherService])
], VoucherController);
