...
let AssetService = class AssetService {
    ...
    async createAsset(dto, UserId) {
        ...
        const asset = await this.prisma.asset.create({
            data: {
                ...
                CurrentValue: new client_1.Prisma.Decimal(dto.PurchasePrice),
    ...
    async listAssets(dto) {
        ...
        const assetsWithDepreciation = await Promise.all(assets.map(async (asset) => {
    ...
    async updateAsset(assetId, dto, UserId) {
        ...
        const updated = await this.prisma.asset.update({
            ...
            data: {
                ...
                CurrentValue: dto.CurrentValue !== undefined ? new client_1.Prisma.Decimal(dto.CurrentValue) : undefined,
                Location: dto.Location,
                AssignedTo: dto.AssignedTo,
                SerialNumber: dto.SerialNumber,
                Description: dto.Description,
    ...
    async getAssetValuation(dto) {
        ...
        for (const asset of assets) {
            ...
            valuationResults.push({
                ...
                PurchasePrice: (0, number_1.number)(asset.PurchasePrice),
    ...
    async disposeAsset(assetId, dto, UserId) {
        const asset = await this.prisma.asset.findUnique({
            where: { ID: assetId },
        ...
        const updated = await this.prisma.asset.update({
            ...
            data: {
                ...
                CurrentValue: new client_1.Prisma.Decimal(0),
    ...
    async TransferAsset(assetId, dto, UserId) {
        const asset = await this.prisma.asset.findUnique({
            where: { ID: assetId },
        });
        if (!asset) {
    ...
    async calculateSingleAssetDepreciation(assetId, asOfDate) {
        const asset = await this.prisma.asset.findUnique({
            where: { ID: assetId },
            include: {
                Category: true,
        ...
        const monthsElapsed = (endDate.getFullYear() - PurchaseDate.getFullYear()) * 12 +
    ...
    formatAsset(asset) {
        return {
            ...
            PurchaseDate: asset.PurchaseDate,
            PurchasePrice: (0, number_1.number)(asset.PurchasePrice),
            currentValue: (0, number_1.number)(asset.CurrentValue),
