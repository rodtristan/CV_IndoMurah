...
export default function CashOutPage() {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.delete("cash-out", selected.ID).catch(() => ({}));
      setShowDelete(false);
      setSelected(null);
      fetchData();
  ...
  return (
    <PageWrapper>
      <Card className="p-4">
        ...
        />
        <div className="mt-4">
          <DataTable data={cashOuts} columns={columns} loading={loading} selectedId={selected?.ID ?? null} onRowClick={setSelected} emptyMessage="Tidak ada data kas keluar" />
        </div>
      </Card>

      <Modal open={showForm} onClose={() => setShowForm(false)} title={(form as any).id ? "Ubah Kas Keluar" : "Kas Keluar Baru"} size="md"
