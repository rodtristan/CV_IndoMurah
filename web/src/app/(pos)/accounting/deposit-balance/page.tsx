...
export default function DepositBalancePage() {
  ...
  const [rows, setRows] = useState<{ name: string; inn: number; out: number }[] | null>(null);
  const [loading, setLoading] = useState(false);
  const opts = useMemo(() => parties.map((p: Row) => ({ value: p.ID, label: p.Name })), [parties]);
  const cfg = type === "customer" ? { ep: "customer-deposit", key: "CustomerID", out: "DPOUT" } : { ep: "supplier-deposit", key: "SupplierID", out: "DBOUT" };

  const process = async () => {
    setLoading(true);
    try {
      const r = await api.get<Row[]>(cfg.ep, { $take: 2000, ...(partyId ? { $where: { [cfg.key]: Number(partyId) } } : {}) }, { skipCache: true });
      const m = new Map<number, { name: string; inn: number; out: number }>();
      for (const d of r.data ?? []) {
        const p = parties.find((x: Row) => x.ID === d[cfg.key]);
        const e = m.get(d[cfg.key]) ?? { name: p?.Name ?? String(d[cfg.key]), inn: 0, out: 0 };
        if (String(d.Code).startsWith(cfg.out)) e.out += num(d.Amount); else e.inn += num(d.Amount);
        m.set(d[cfg.key], e);
      }
      setRows([...m.values()]);
      ...
      <KCard>
        ...
        <div className="overflow-x-auto border border-[#c9d0d8]">
          <table className="w-full text-[13px]">
            <thead><tr className="border-b border-[#c9d0d8] bg-[#f5f6f8]">{["No", type === "customer" ? "Pelanggan" : "Supplier", "Deposit Masuk", "Deposit Keluar", "Saldo Akhir"].map((h, i) => <th key={h} className={cn("px-3 py-2 font-medium", i >= 2 ? "text-right" : "text-left")}>{h}</th>)}</tr></thead>
            <tbody>
              {(!rows || rows.length === 0) && <tr><td colSpan={5} className="h-32 text-center text-[#9aa3ad]">{rows ? "No data" : "Klik Proses untuk menampilkan saldo"}</td></tr>}
              {(rows ?? []).map((r, i) => <tr key={i} className="border-b border-[#eceff2]"><td className="px-3 py-1.5">{i + 1}</td><td className="px-3 py-1.5">{r.name}</td><td className="px-3 py-1.5 text-right">{fmt(r.inn)}</td><td className="px-3 py-1.5 text-right">{fmt(r.out)}</td><td className="px-3 py-1.5 text-right font-medium">{fmt(r.inn - r.out)}</td></tr>)}
            </tbody>
          </table>
        </div>
      </KCard>
