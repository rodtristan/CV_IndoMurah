...
export default function SalePointsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("sale-point", { $search: search || undefined, $select: "id,code,name,address,phone,isActive" } as any).catch(() => ({ success: false, data: { data: [] } } as any));
  ...
  const handleSave = async () => {
    if ((form as any).id) {
      await api.patch("sale-point", (form as any).id, form).catch(() => ({}));
    } else {
      await api.post("sale-point", form).catch(() => ({}));
    }
    setShowForm(false);
    fetchData();
  };
